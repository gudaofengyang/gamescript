...
# 主窗口名为jiaoben，设置主窗口背景图片为backscene.jpg，设置背景图片的拉伸方式为平铺，图片透明度为0.5，
ui.setStyleSheet(
    "#jiaoben{background-image:url(../art/scence/backscene.jpg);background-repeat:repeat;background-color:rgba(0,0,0,"
    "0.5);}")
...
a传入range方法获取数组
创建一个loop()方法,
执行kill-one方法a次,每次间隔b秒
kill-one方法来自别的自定义模块kill-monster.py
'''
